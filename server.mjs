    ...
    for (const room of Object.values(legacy.sessions || {})) {
      ...
      for (const player of Object.values(room.players || {})) {
        ...
        await tx.player.create({ data: {
          ...
          joinKey: player.joinKey || null, idleImage: player.idleImage || null, talkingImage: player.talkingImage || null, mediaMode: player.mediaMode || "png",
...
app.put("/api/sessions/:sessionId/players/:playerId", requireHost, requireCsrf, async (req, res) => {
  ...
  const player = await prisma.player.upsert({ where: { id }, create: {
    ...
    pinned: req.body?.pinned === undefined ? true : Boolean(req.body.pinned), mediaMode: "png", presence: { create: { source: "manual" } },
  }, update: {
    ...
    pinned: req.body?.pinned === undefined ? current?.pinned ?? true : Boolean(req.body.pinned),
  } });
...
app.post("/api/join/:sessionId/:joinToken", joinLimit, async (req, res) => {
  ...
  const player = await prisma.player.create({ data: {
    ...
    pinned: false, joinKey: token(24), mediaMode: req.body?.mediaMode === "webcam" ? "webcam" : "png", presence: { create: { source: "browser" } },
  } });
